...
public class ListRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ListRecyclerViewAdapter(Context context, List<Object> items, RecyclerViewClickListener listener) {
        this.items = items;
        mContext = context;
        mListener = listener;
    }
    ...
    private void configureMovieViewHolder(MovieViewHolder holder, int position) {
        ...
        if (movie != null) {
            ...
            holder.tvTitleYear.setText((position+1) + ". " + movie.getTitle() + " (" + movie.getYear() + ")");
