...
public class GetUserRatingJsonData extends GetRawData {
    private String LOG_TAG = GetUserRatingJsonData.class.getSimpleName();
    ...
    public GetUserRatingJsonData(String accessToken) {
        super(null);
    ...
    public void processResult(){
        ...
        try {
            JSONArray jsonArray = new JSONObject(getmData()).getJSONArray(Constants.DATA)
                    ...
                    .getJSONObject(Constants.RATED_MOVIES).getJSONArray(Constants.DATA);
            for (int i = 0; i< jsonArray.length(); i++){
                JSONObject jsonMovie = jsonArray.getJSONObject(i).getJSONObject(Constants.ATTRIBUTES);
                Long id = jsonMovie.getLong(Constants.MOVIE_ID);
                String rating = jsonMovie.getString(Constants.RATING);
                JSONObject jsonMovieAtt = jsonMovie.getJSONObject(Constants.MOVIE).getJSONObject(Constants.DATA)
                ...
                Movie movie = new Movie(id, title, year, null, null, null, null,
                        null, null, null, null, poster, rating, null, null, null);
                mMovies.add(movie);
            }
